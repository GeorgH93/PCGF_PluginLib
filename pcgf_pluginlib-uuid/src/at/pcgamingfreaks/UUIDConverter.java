...
public final class UUIDConverter
{
	...
	{
		at.pcgamingfreaks.UUID.NameChange[] history = MOJANG_RESOLVER.getNameHistory(toUUID(uuid));
		if(history == null) return null;
		NameChange[] changeHistory = new NameChange[history.length];
		for(int i = 0; i < history.length; i++)
		{
			changeHistory[i].name = history[i].name;
			changeHistory[i].changedToAt = history[i].changedToAt;
		}
